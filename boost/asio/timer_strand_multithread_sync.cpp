 ...
 * have a pool of threads calling io_service::run(). However, as this allows handlers to execute concurrently, 
 * we need a method of synchronisation when handlers might be accessing a shared, thread-unsafe resource.
