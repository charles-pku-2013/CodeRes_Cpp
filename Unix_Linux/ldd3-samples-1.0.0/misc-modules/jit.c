...
int delay = HZ / 2; /* the default delay, expressed in jiffies */
	      ...
	      int len, int *eof, void *data)
{
	...
	printk(KERN_INFO "process %i (%s) read the proc file.\n",
...
/*wait for finishing writting the buffer and then output all, not one by one*/
...
int __init jit_init(void)
{
	...
	create_proc_read_entry("jitnormal", 0, NULL, jit_fn, (void*)JIT_NORMAL);
...
void __exit jit_cleanup(void)
{
	...
	remove_proc_entry("jitnormal", NULL);
