...
const std::string&
DemoDict::Lookup(const std::string &key) {
    static std::string null;
    auto it = dict_.find(key);
    return (it == dict_.end() ? null : it->second);
}

std::string DemoDict::Description() const {
    return absl::StrJoin(dict_, ", ", absl::PairFormatter("="));
}
...
CreateDemoDict(const std::string& path, const std::shared_ptr<demo::ServableInfo> &info,
