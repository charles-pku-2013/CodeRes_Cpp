...
TraceLoggerManager::~TraceLoggerManager()
{ Stop(); }

bool TraceLoggerManager::Init(hocon::shared_config conf) {
    try {
        _Init(conf);
    } catch (const std::exception& ex) {
        LOG(ERROR) << "TraceLoggerManager init fail: " << ex.what();
        return false;
    } catch (...) {
        LOG(ERROR) << "TraceLoggerManager init fail!";
        return false;
    }
    inited_ = true;
    return true;
...
void TraceLoggerManager::_Init(hocon::shared_config conf) {
    if (!conf) {
        throw std::runtime_error("TraceLoggerManager init fail for NULL conf");
    }

    sample_rate_ = conf->has_path("sample_rate") ? conf->get_long("sample_rate") : GetSampleRate();
...
RequestTraceLoggerObj TraceLoggerManager::NewLogger() {
    size_t sample_rate = GetSampleRate();
    if ( !inited_ || sample_rate == 0) { return TraceLogger::pointer(); }   // disable logging
    return (req_cnt_++ % sample_rate == 0 ? std::make_shared<TraceLogger>(common_record_) : TraceLogger::pointer());
}

void TraceLoggerManager::Stop() {
    inited_ = false;
    if (file_sink_) {
...
std::string TraceLoggerManager::DebugString() const {
    return absl::StrFormat("{sample_rate:%lu,logdir:%s,log_filename:%s,rotation_size:%ld,rotation_time:%s,"
                                ...
                                GetSampleRate(), logdir_, log_filename_, rotation_size_,
...
void TraceLogger::Flush() {
    ...
    if (!record_.empty()) {
        auto& logger = TraceLoggerManager::Instance().Logger();
