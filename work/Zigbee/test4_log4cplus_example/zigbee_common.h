...
#define	ZIGBEE_VERSION		"0.1.1-0607c"
...
extern uint16 to_u16( const void *ptr );
extern uint32 to_u32( const void *ptr );
extern int16 to_16( const void *ptr );
...
END:		
	...
	T add_unique_replace( const T &value, const Predicate &pred )
	{
		...
		retval = BaseType::add_unique_replace( value, pred );
END:		
		pthread_mutex_unlock( &lock );
		return retval;
	}
...
typedef boost::shared_ptr<AtCmdCallback> AtCmdCallbackPtr;
