...
//	pSpecDev->setEndpoint( pFrame->srcEp() );		//!! 0xFF
	...
	pDev = pSpecDev.get();
	zigbeeDevCandListForUpdate.add_unique_replace( pSpecDev.release(), ZDevCmp(finder, pDev->addr()) );
	sprintf( cmdStr, "atieee %04x", pFrame->addr() );
	ret = zigbee_at_cmd( API_ATCMD, NULL, cmdStr, 0 );
...
int parseNewJoin( void *arg, Callback *cb )
{
    ...
    zigbeeDevCandListForNewJoin.add_unique_replace( pDev, ZDevCmp(ZDEV_FIND_BY_ADDR, pDev->addr(), NULL) );
...
//	DBG( "Now add new dev to devList." );
	...
	zigbeeDevCandListForNewJoin.add_unique_replace( pDev, ZDevCmp(finder, pDev->addr(), NULL, NULL, pFrame->profileID(), pFrame->srcEp()) );
...
//#endif
	...
	while( (cb = broadcastCbList.find_remove( GeneralCallbackCmp(method, NULL, thisDev->profileId(), 
						ZCL_CLUSTER_ID_GEN_BASIC, 0, thisDev->endpoint(), reqFrame.seqNO(), (uint8)ZclBase::CB_READ_ATTR)) ) ) {
//		DBG( "zigbee_update_dev_list() delete old callback from broadcastCbList." );
		delete cb;
	}
...
//	read_all_ascii(fd);
	...
	atCmdCallbackTable["atieee"] = new AtCmdCallback( "EREPOIEEE", parseAtIeee, 0 );
...
//	buf = (uint8*)malloc( zclPayloadLen + 5 );
	buf = (uint8*)malloc( zclPayloadLen + 20 );
	if( !buf )
		goto FAIL;
