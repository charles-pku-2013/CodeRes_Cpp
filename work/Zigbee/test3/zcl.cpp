...
int zclDataTypeLen(uint8 dataType)
{
	switch(dataType)
	{
...
//	uint16			attrID;
//    
//    DBG( "ZclBase::readAttr() clusterID = %04x", clusterId );
//    
//    addrData = dev.addr();
//    make_address( &addr, SHORT, &addrData );
//    
...
//    }
//    
////    DBG( "ZclBase::readAttr() got callback data." );
...
//	// get status
//    attr.status = *pRspData++;
//	attr.dataType = *pRspData++;
//	attr.setData( pRspData, pResultFrame->payload() + pResultFrame->payloadLen() );  
//	
//    return ret;
//}
...
//	DBG2( "%s", pResultFrame->toString() );
    ...
    while( pRspData < pResultFrame->payload() + pResultFrame->payloadLen() ) {
        ...
        pAttr = std::find_if( attrArray, attrArray + arrayLen,
                        ZclGenCmp<ZclAttr>(ZCL_FIND_BY_ID, attrID) );
...
int ZclBase::updateAttr( ZigbeeDevice &dev, uint16 attrID, int timeout )
{
	...
	pAttr = std::find_if( attrList, attrList + attrListLen, ZclGenCmp<ZclAttr>(ZCL_FIND_BY_ID, attrID) );
