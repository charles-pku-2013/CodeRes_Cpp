/*
 * https://joyspace.jd.com/pages/sF9xiRJVNOWtAnOmcK9I
 * https://joyspace.jd.com/pages/M2sFMTm5DXo6W3J8OStj
 */
#pragma once
#include <memory>
#include <atomic>
...
#include "common/canvas/spinlock.h"
...
class RequestTraceLogger {
    friend class RequestTraceLoggerManager;
    friend class RequestTraceLoggerObj;
 public:
    using pointer = std::shared_ptr<RequestTraceLogger>;

 public:
    explicit RequestTraceLogger(const Json::Value& record)
            : record_(record) {}
    virtual ~RequestTraceLogger();
...
/*
 * wrapper for RequestTraceLogger
 * 实际工程中使用这个，隐藏对logger是否为空判断
 */
class RequestTraceLoggerObj {
 public:
    RequestTraceLoggerObj() = default;
    RequestTraceLoggerObj(const RequestTraceLogger::pointer& logger)
                    : logger_(logger) {}
 ...
 private:
    RequestTraceLogger::pointer logger_;
};

/*
 * logger管理类，全局单实例
 */
class RequestTraceLoggerManager final {
 ...
 public:
    static RequestTraceLoggerManager& Instance() {
        static RequestTraceLoggerManager inst;
        return inst;
    }
 ...
 private:
    ...
    std::size_t sample_rate_ = 1;    // sample every N req, 0: disable, 1 (default): every req
 ...
 private:
    ...
    RequestTraceLoggerManager() : logger_(boost::log::keywords::channel = LOG_CHANNEL) {}
    ~RequestTraceLoggerManager();
    RequestTraceLoggerManager(const RequestTraceLoggerManager&) = delete;
    RequestTraceLoggerManager(RequestTraceLoggerManager&&) = delete;
    RequestTraceLoggerManager& operator=(const RequestTraceLoggerManager&) = delete;
    RequestTraceLoggerManager& operator=(RequestTraceLoggerManager&&) = delete;
};
